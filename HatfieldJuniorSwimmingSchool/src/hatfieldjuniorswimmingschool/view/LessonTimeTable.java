...
public class LessonTimeTable extends javax.swing.JFrame {
    ...
    public LessonTimeTable() {
        ...
        //store coach name in select button
        storeCoachInSelectButton();
        //dispaly lesson in jtable
        displayLessonDetails();
        //set booking panel false
        bookingPanel.setVisible(false);
    }
    ...
    private void initComponents() {
        ...
        bookLesson.setText("Book Lesson ");
    ...
    private void coachSelectActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_coachSelectActionPerformed
        //filter table by seoect coach name
    ...
    private void lessonDetailsMouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_lessonDetailsMouseReleased
        //get lesson details on selection if user id not 0
        if (userId != 0) {
    ...
    public void displayLessonDetails() {
        ...
        for (Lesson lessons : lesson_details) {
            //get all lesson details from csv file 
    ...
    public void filterLessonTable(String day_select, String coach_select, String grade_select) {
       ...
       //empty jtable if not data found
        if (!dataFound) {
        ...
        if (!(user_grade == lesson_grade || user_grade1 == lesson_grade)) {
            if (userId != 0) {
                ...
                JOptionPane.showMessageDialog(this, "You are learner of grade level " + user_grade + "\n"
                        + "you can book only level " + user_grade + " or level " + user_grade1, "Alert", JOptionPane.WARNING_MESSAGE);
            }
            bookingPanel.setVisible(false);
        }
    }
    ...
    public void bookingLesson() {
        //change lesson id into integer
        int lessonId = Integer.parseInt(lesson_id.substring(12));
        //get result after booking lesson
        boolean bookLessons = new BookingController().userBooking(lessonId, userId);
        if (bookLessons) {
            lesson_id = lesson_id.substring(12);
            new LessonController();
            //call update seat functioality from lesssonController
            new LessonController().updateSeatAvailablity(lesson_id, "Booked");
            JOptionPane.showMessageDialog(this, "Booking Successfully", "Alert", JOptionPane.WARNING_MESSAGE);
            this.setVisible(false);
            //open user dashboard after booking lesson
            new UserDashboard(userId).setVisible(true);
            //clear select panel after booking lesson
            clearSelectedPanle();

        } else {
            JOptionPane.showMessageDialog(this, "This lesson already booked", "Alert", JOptionPane.WARNING_MESSAGE);
        }
    }
